...
func doTask(taskType string,
	runAfterTime string,
	params map[string]string,
	myLogFile string, dir string,
	mainFile string, cmdArgs string ) error {
	...
	err = cmd.Wait()
	if err != nil {
		errorLog( myLogFile, err )
	}

	if errStderr != nil || errStdout != nil {
		errorLog(myLogFile,errStdout, errStderr)
	}

	outStr, errStr := string(stdout), string(stderr)
	infoLog( myLogFile, fmt.Sprintf( "\nout:\n%s\n", outStr ) )
	if errStr != "" {
		errorLog( myLogFile, fmt.Sprintf( "\nerr:\n%s\n", errStr ))
	}
	return nil
}
...
func ansyTaskWorker(queue string, args ...interface{}) error {
	...
	myLogFile = ""
	if _, ok := params["logFile"]; ok{
		myLogFile = params["logFile"]
	}

	//打印日志
	infoLog( myLogFile, fmt.Sprintf( "From Redis Key : %s; Args: %v\n", queue, args[0] ) )
	...
	if runAfterTime != "0" {
		afterChan <- 1 //任务加1
		go doTask(taskType, runAfterTime,params,myLogFile,dir,mainFile,cmdArgs)
	}else{
		doTask(taskType, runAfterTime,params,myLogFile,dir,mainFile,cmdArgs)
	}
